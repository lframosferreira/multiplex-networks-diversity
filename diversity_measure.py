...
def less_contribute_rank(node_distance_distributions: npt.NDArray[np.float_], trasition_matrices: npt.NDArray[np.float_]) -> npt.NDArray[np.int_]:
    ...
    number_of_layers: np.int_ = trasition_matrices.shape[0]
    combinations: list[tuple[int]] = list(itertools.combinations(np.arange(number_of_layers), 2))
    layer_difference_matrix: npt.NDArray[np.float_] = np.zeros(shape=((number_of_layers, number_of_layers)), dtype=np.float_)
